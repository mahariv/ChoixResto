...
{
    // pour retrouver la base de donnée qui a été généré par le test unitaire il faut utiliser le nom de serveur :
    // (localdb)\MSSQLLocalDB

    // pour ne pas que nos tests empoetes sur notre BDD de production on va rajouter dans App.config la balise suivante :
//    <connectionStrings>
...
//</connectionStrings>
    ...
    public class DalTests
    {
        ...
        //// testInitialse permet de lancer la methode sur laquel il est placé avant chaque test, ici il s'agit d'une méthode pour réinitialiser une BDD
        ////[TestInitialize]
        ...
        //{
        //    // permet de réinitialiser la base de donnée de BDDContext
        //    IDatabaseInitializer<BddContext> init = new DropCreateDatabaseAlways<BddContext>();
        //    Database.SetInitializer(init);
        //    init.InitializeDatabase(new BddContext());
        ...
        //    {
        //        dal.CreerRestaurant("Fourchette", "01 43 54 67 35");
        //        List<Restaurant> listeRestaurants = dal.ObtenirTousLesRestaurants();

        //        Assert.IsNotNull(listeRestaurants);
        //        Assert.AreEqual(1, listeRestaurants.Count);
        //        Assert.AreEqual("Fourchette", listeRestaurants[0].Nom);
        //        Assert.AreEqual("01 43 54 67 35", listeRestaurants[0].Telephone);
        //    }
        //}

        //[TestMethod]
        //public void ModifierREstaurantFourchetteEnCouteau_000000000()
        //{
        ...
        //        int id = listeRestaurants.FirstOrDefault(elt => elt.Nom == "Fourchette").Id;

        //        dal.ModifierRestaurant(id, "Couteau", "00 00 00 00 00");

        //        listeRestaurants = dal.ObtenirTousLesRestaurants();

        //        Restaurant myRestaurant = listeRestaurants.FirstOrDefault(elt => elt.Id == id);

        //        Assert.AreEqual("Couteau", myRestaurant.Nom);
        //        Assert.AreEqual("00 00 00 00 00", myRestaurant.Telephone);
        ...
        //#endregion avant Tp
            ...
            public void CreerRestaurant_AvecUnNouveauRestaurant_ObtientTousLesRestaurantsRenvoitBienLeRestaurant()
            {
                ...
                List<Restaurant> restos = dal.ObtenirTousLesRestaurants();
            ...
            Assert.AreEqual(1, resultats2[0].NombreDeVotes);
            Assert.AreEqual("Resto pinière", resultats2[0].Nom);
            Assert.AreEqual("0102030405", resultats2[0].Telephone);
            Assert.AreEqual(1, resultats2[1].NombreDeVotes);
            Assert.AreEqual("Resto pinambour", resultats2[1].Nom);
            Assert.AreEqual("0102030405", resultats2[1].Telephone);
            Assert.AreEqual(2, resultats2[2].NombreDeVotes);
            Assert.AreEqual("Resto mate", resultats2[2].Nom);
            Assert.AreEqual("0102030405", resultats2[2].Telephone);
